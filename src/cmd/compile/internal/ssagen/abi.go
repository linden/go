...
// TODO: lot of duplicate with `GenWasmExportWrapper`.
func genAsyncWasmExportWrapper(wrapped *ir.Func) {
	pos := base.AutogeneratedPos
	sym := &types.Sym{
		Name:     wrapped.WasmExport.Name + "_resume",
		Linkname: wrapped.WasmExport.Name + "_resume",
	}
...
func GenWasmExportWrapper(wrapped *ir.Func) {
	...
	if wrapped.WasmExport.Async {
		genAsyncWasmExportWrapper(wrapped)
	}
...
func setupWasmExport(f, wrapped *ir.Func) {
	...
	we.Results = resultsToWasmFields(wrapped, "go:wasmexport", abiInfo, abiInfo.OutParams())
	we.Async = wrapped.WasmExport.Async
